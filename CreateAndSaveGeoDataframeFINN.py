...
def CreateDataFrameFINN(Lat_min,Lat_max,Long_min,Long_max,RegionName,Num,Datatype = ''):
    ...
    if 'Month' in Datatype:
        dp = datapath + "/FINN/FINN1.5_CO2_2009_2020_daily_1x1_monthMeans.nc"
    else:
        dp = datapath + "/FINN/netcdf_files/emissions-finnv1.5_daily_CO2_*.nc"
    for num, filepath in enumerate(glob.glob(dp)):
        #for num, filepath in enumerate(glob.glob(datapath + "/FINN/*.nc")):
    ...
    df.to_pickle(datapath + "/FINN/dataframes/DF1_"+RegionName+str(Num)+".pkl")
    ...
    gdf.to_pickle(datapath + "/FINN/dataframes/GDF1_"+RegionName+str(Num)+".pkl")
