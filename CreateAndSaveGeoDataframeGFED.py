...
Create GeoDataframe of GFED data
@author: eschoema
"""
...
if __name__ == "__main__": #if this script is executed, not executed if function of this script is imported in another script
    ...
    if not os.path.isfile(savepath+"GDFnew_"+typelist[datatype]+"_"+RegionName+".pkl"):
        gdf = createDataFrameGFED(savepath,savepath,datatype,Long_min,Long_max,Lat_min,Lat_max,RegionName, UseTranscom)
    else:
    ...
    MonthMeans = getMonthSum(gdf,datetime.date(2009,1,1),datetime.date(2019,12,31),'total_emission')
